...
int InitFileExist(QString filepath)
{
    ...
    if(!inifile.exists())
    {
//        QLog_("init", ErrorLevel, QString("/ivfdinit.ini not exisit"));
...
{
    if(!InitFileExist(QString("/autorun.sh")))
    {
        return 0;
    }
...
int GetEnviroment(QString mainkey, QString subkey, QVariant *subkeyval)
{
    ...
    if(!InitFileExist(QString("/ivfdinit.ini")))
    {
        return 0;
    }

    QSettings initsetting("/ivfdinit.ini", QSettings::IniFormat);
    QStringList mainkeylist = initsetting.childGroups();
